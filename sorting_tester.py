...
class SortingTester:
    ...
    def __init__(self, min_len_log_2, max_len_log_2, rel_cluster_size = DEFAULT_RELATIVE_CLUSTER_SIZE):
        ...
        for i in range(min_len_log_2, max_len_log_2 + 1):
            t = 1 << i
    ...
    def unorder_range(range_size, output_len):
        ...
        init_list = list(range(range_size))

        return [init_list.pop( int( (range_size - i) * random.random() ) ) for i in range(output_len)]
    ...
    def cluster_unorder(min_cluster_size, max_cluster_size, list_size):
        ...
        while len(output_list) < list_size:
            ...
            output_list.extend([start_value + i
                       for i in range(min_cluster_size + int(random.random() * (max_cluster_size - min_cluster_size)))])
    ...
    def create_list_to_sort(list_size, rel_cluster_size):
        ...
        return [
            ...
            (0.8, 'unordered_0.2', SortingTester.partly_unorder_list(list(range(list_size)), 0.2)),
            (0.6, 'unordered_0.4', SortingTester.partly_unorder_list(list(range(list_size)), 0.4)),
            (0.4, 'unordered_0.6', SortingTester.partly_unorder_list(list(range(list_size)), 0.6)),
            (0.2, 'unordered_0.8', SortingTester.partly_unorder_list(list(range(list_size)), 0.8)),
            (0.0, 'unordered', SortingTester.unorder_range(list_size, list_size)),
            (-0.2, 'invert_unordered_0.8', SortingTester.partly_unorder_list(list(range(list_size - 1, -1, -1)), 0.8)),
            (-0.4, 'invert_unordered_0.6', SortingTester.partly_unorder_list(list(range(list_size - 1, -1, -1)), 0.6)),
            (-0.6, 'invert_unordered_0.4', SortingTester.partly_unorder_list(list(range(list_size - 1, -1, -1)), 0.4)),
            (-0.8, 'invert_unordered_0.2', SortingTester.partly_unorder_list(list(range(list_size - 1, -1, -1)), 0.2)),
